...
def plot_running_average_al_files():
    ...
    for filename in os.listdir(directory):
        # if filename == "learningRate=0.001_batchSize=128_memorySize=5000_prioritizedMemory=True_lrs=True_smartReward=False_betaIncrement=Truedueling=True_double=True_gradientClipping=True_18:55.csv" or filename == "learningRate=0.001_batchSize=128_memorySize=5000_prioritizedMemory=True_lrs=True_smartReward=True_betaIncrement=Truedueling=True_double=True_gradientClipping=True_16:57.csv":
        file_path = os.path.join(directory, filename)
        data = pd.read_csv(file_path)
        scores = data['score']
        all_scores.append(scores)
        running_avg = scores.rolling(window=window_size).mean()
        plt.plot(running_avg, linewidth=0.5)

    # Calculate the mean of all scores
    mean_scores = np.mean(all_scores, axis=0)

    # Calculate the rolling mean of the mean scores
    mean_rolling_mean = pd.Series(mean_scores).rolling(window=100).mean()
